import { ExternalLink, Building2, Home, Store } from "lucide-react";
      ...
      <div className="container mx-auto px-4 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          {projects.map((project, index) => (
            <div
              key={project.title}
              className={`group bg-card rounded-2xl border border-border overflow-hidden hover:border-accent/30 transition-all duration-500 hover:shadow-xl ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
              style={{ transitionDelay: `${index * 150}ms` }}
            >
              {/* Project Preview */}
              <div className={`relative h-48 bg-gradient-to-br ${project.gradient} overflow-hidden`}>
                {/* Browser Mockup */}
                <div className="absolute inset-4 bg-card rounded-lg shadow-lg overflow-hidden border border-border/50">
                  {/* Browser Header */}
                ...
                {/* Icon Overlay */}
                <div className="absolute top-4 right-4 w-10 h-10 rounded-xl bg-background/90 flex items-center justify-center shadow-md">
